...
function handlePartnerDocumentUploadError(err, req, res, next) {
  if (!err) return next();

  if (err.code === "LIMIT_FILE_SIZE") {
    return res.status(400).json({
      message: "Document too large (max 10MB)",
      code: "INVALID_PARTNER_DOCUMENT"
    });
  }

  if (err.code === "INVALID_PARTNER_DOCUMENT" || err.message === "Only image or PDF files are allowed") {
    return res.status(400).json({
      message: err.message,
      code: "INVALID_PARTNER_DOCUMENT"
    });
...
module.exports = {
  ...
  partnerDocumentsUploadsDir: uploadsDir,
